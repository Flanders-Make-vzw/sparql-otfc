...
async function preprocess(q, context) {
	...
	if (q.type() === 'SELECT') {
		...
		if (computed.length) {
			...
			let fx = async (qx, subjects) => {
				return await filterComputedSubjects(qx, context, subjects);
			};
...
async function filterComputedSubjects(q, context, subjects) {
	const limit = 10000;
	const engine = new comunica.QueryEngine();
...
async function computePredicates(q, context, store) {
	...
	for (const t of q.whereTriples()) {
		...
		if (predicates.includes(p)) {
			...
			console.log(chalk.yellow.bold('Computed ' + triples.length + ' triples'));
			computed.push(p);
